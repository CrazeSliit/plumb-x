...
import { sampleValvesData } from '../../../data/sampleData';
    ...
    if (typeof window !== 'undefined') {
      ...
      if (storedItems) {
        ...
        if (Array.isArray(parsedItems)) {
          // Filter to only include valve items
          const valveItems = parsedItems.filter(item => 
            item.category === 'valves' || 
            item.category === 'Valves' ||
            (item.name && item.name.toLowerCase().includes('valve'))
          );
          
          console.log(`Found ${valveItems.length} valve items in localStorage`);
          return valveItems.map(item => ({
            id: item.id || item.itemCode,
            name: item.name || item.itemName,
            type: item.type || 'Ball',
            material: item.material || 'Brass',
            ...
            image: item.image || '/image/valve-placeholder.jpg',
...
export default function ValvesPage() {
  ...
  useEffect(() => {
    ...
    const fetchItems = async () => {
      ...
      const storedValveItems = loadStoredItems();
      
      // Use the imported sample data
      const data = sampleValvesData;
      
      // Combine localStorage items with sample data
      let combinedData = [];
      
      if (storedValveItems && storedValveItems.length > 0) {
        // Start with stored items
        combinedData = [...storedValveItems];
        
        // Add sample items that don't conflict with stored items
        const existingIds = new Set(storedValveItems.map(item => item.id));
        const uniqueSampleItems = data.filter(item => !existingIds.has(item.id));
        
        combinedData = [...combinedData, ...uniqueSampleItems];
        console.log(`Showing ${storedValveItems.length} stored items and ${uniqueSampleItems.length} sample items`);
      } else {
        combinedData = data;
    ...
    <div className="min-h-screen bg-gray-100 p-6">
      <div className="max-w-7xl mx-auto">
        ...
        <nav className="flex mb-4" aria-label="Breadcrumb">
          <ol className="inline-flex items-center space-x-1 md:space-x-3">
            ...
            <li aria-current="page">
              <div className="flex items-center">
                <ChevronRightIcon className="w-5 h-5 text-gray-400"/>
                <span className="ml-1 text-sm font-medium text-gray-500 md:ml-2">Valves</span>
              </div>
            </li>
          </ol>
        </nav>
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">Plumbing Valves</h1>
            <p className="text-gray-600 mt-1">Browse our selection of high-quality valves for flow control and regulation</p>
          </div>
          <div className="mt-4 md:mt-0 flex flex-wrap gap-2">
            <Link href="/add_new?category=valves" className="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-black bg-[#fdc501] hover:bg-yellow-400">
              <PlusCircleIcon className="mr-2 h-5 w-5" />
              Add New Valve
            </Link>
            <Link href="/inventory_report?category=valves" className="inline-flex items-center px-4 py-2 border border-gray-300 text-sm font-medium rounded-md shadow-sm text-gray-700 bg-white hover:bg-gray-50">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          <div className="bg-white rounded-lg shadow p-6 border-l-4 border-[#fdc501]">
            <div className="flex justify-between items-center">
              <div>
                <p className="text-sm font-medium text-gray-600">Total Valves</p>
          ...
          <div className="bg-white rounded-lg shadow p-6 border-l-4 border-green-500">
            <div className="flex justify-between items-center">
              <div>
                <p className="text-sm font-medium text-gray-600">Valve Types</p>
        ...
        <div className="bg-white rounded-lg shadow mb-6 p-4">
          <div className="flex flex-col md:flex-row justify-between items-center gap-4">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <button
                onClick={() => setFilter('Brass')}
                className={`px-3 py-1.5 text-sm rounded-lg ${
                  filter === 'Brass' ? 'bg-[#fdc501] text-black' : 'bg-gray-100 text-gray-700'
                }`}
              >
                Brass
              </button>
              <button
                ...
                className={`px-3 py-1.5 text-sm rounded-lg ${
                  filter === 'PVC' ? 'bg-[#fdc501] text-black' : 'bg-gray-100 text-gray-700'
                }`}
              >
                PVC
              </button>
              <button
                onClick={() => setFilter('Cast Iron')}
                className={`px-3 py-1.5 text-sm rounded-lg ${
                  filter === 'Cast Iron' ? 'bg-[#fdc501] text-black' : 'bg-gray-100 text-gray-700'
                }`}
              >
                Cast Iron
              </button>
              <button
                onClick={() => setFilter('Stainless Steel')}
                className={`px-3 py-1.5 text-sm rounded-lg ${
                  filter === 'Stainless Steel' ? 'bg-[#fdc501] text-black' : 'bg-gray-100 text-gray-700'
                }`}
              >
                Stainless Steel
              </button>
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-lg shadow p-8 text-center">
            ...
            <h3 className="mt-2 text-lg font-medium text-gray-900">No valves found</h3>
            ...
            {filteredItems.map(item => (
              <div key={item.id} className="bg-white rounded-lg shadow-md overflow-hidden">
                <div className="relative h-48 bg-gray-200">
                  <img 
                    src={item.image || '/image/valve-placeholder.jpg'} 
                    alt={item.name}
                    className="w-full h-full object-cover"
                    onError={(e) => {
                      e.target.onerror = null;
                      e.target.src = '/image/valve-placeholder.jpg';
                    }}
                  />
                  <div 
